...
"""The Xenia executable to spawn (env `XENIA_BIN`, default `/opt/xenia/AppRun`)."""
DATA_DIR = Path(os.environ.get("XENIA_DATA_DIR", "/config/xenia"))
"""Xenia's storage root, the save root (env `XENIA_DATA_DIR`, default `/config/xenia`).
...
"""The emulator log file (env `XENIA_LOG_PATH`, default `/config/xenia.log`)."""

ROM_EXTENSIONS = (".iso", ".xex")
"""XISO dumps and extracted executables; a folder holding several candidates picks by this order.

STFS containers (XBLA titles and Games on Demand installs) have no extension
and are found by layout instead, see `_find_container`.
"""
_ROM_SEARCH_GLOBS = ("*", "*/*")
"""Glob patterns a ROM folder is searched with, one level of wrapper folder deep."""

_CONTAINER_TYPE_DIRS = ("000D0000", "00007000")
"""The content-type directories that hold a bootable game package.
...
"""Glob patterns that reach a package under each supported title folder layout.

They cover the title folder handed over bare (`<TITLE_ID>` as the root), with
the `Content/<XUID>` prefix, and with one more wrapper such as the folder RomM
names after the game.
"""
_STFS_MAGICS = (b"CON ", b"LIVE", b"PIRS")
"""The four-byte signatures an STFS package can start with."""
...
def _is_stfs_package(path: Path) -> bool:
    """Whether `path` starts with an STFS package signature.

    Args:
        path: The file to probe.

    Returns:
        True when the first four bytes are one of `_STFS_MAGICS`, False
...
def _find_container(base: Path) -> Path | None:
    ...
    Args:
        base: The title folder, in any of the layouts `_CONTAINER_GLOBS` covers.

    Returns:
        The resolved package path, or None when the layout holds none.
    """
    ranked = []
...
class Xenia(Emulator):
    """Xbox 360 via Xenia Edge, driven by command line flags and a hard SIGTERM.

    Xenia has no control API of any kind, so the broker boots it with
    `--fullscreen --headless --storage_root=... --discord=false` and the ROM
    path, and the stop is the base SIGTERM then SIGKILL sequence. Xenia
    installs no SIGTERM handler, so SIGTERM is already a hard kill; that is
    safe because guest save writes are write-through to host files, and
    Xenia's own exit path only saves config and flushes the log. The exit
    must be broker-driven anyway: a game exiting to dashboard leaves Xenia
    on a modal dialog, and a guest main-thread exit leaves it idling. A
    signed-in profile must already exist under the storage root or Xenia
    raises a "No Profiles Found" dialog nobody in the stream can dismiss.

    There are no save states: persistence is the game's own save data under
    the content tree, which the archive carries along with the profile store
    the save paths are keyed by. A resume slot is logged and ignored.

    Attributes:
        name: Provider key, `xenia`.
        display_name: Human-readable name.
        save_root: The storage root the content subtree hangs off.
        save_subtrees: The whole `content` tree.
        rom_extensions: Bootable formats; STFS containers are found by layout instead.
        log_path: The emulator log file.
        term_timeout: Inherited 5 s; SIGTERM is already a hard kill (no
            handler installed) and save writes are write-through, so the
            base wait before SIGKILL is only a formality.
    """
    ...
    Saves (`<XUID>/<TITLE_ID>/00000001`), their .header sidecars, and the
    profile store the save paths are keyed by. The delta dump only ships
    files modified since launch, so installed DLC sitting in here does not
    bloat the archive.
    """
    ...
    def resolve_rom_file(self, path: Path) -> Path | None:
        """The path Xenia should boot for `path`.

        Args:
            path: A ROM file, or a title folder.

        Returns:
            The file itself; for a folder, its `default.xex` (an extracted
            dump), else the best-ranked disc or executable up to two levels
            deep, else the STFS package an XBLA or GoD layout holds; None
    ...
    def launch(self, rom_path: Path, resume_slot: int | None) -> None:
        """Boot the game fullscreen and headless under the broker's storage root.

        Args:
            rom_path: The disc image, executable or STFS package to boot.
            resume_slot: Ignored with a log line; Xenia has no save states.
        """
        self.stop()
        if resume_slot:
            log.info(
