...
<Callout type="info">
A `.pkg` ROM, or a `.7z`/`.zip`/`.rar` archive holding one, needs the
extraction cache. Set `SHADPS4_CACHE_ENABLED=true` to boot those formats: the
first launch extracts the ROM into the cache and every later launch reuses
that extraction (see [Configuration](/docs/configuration/emulators)). Leave
the flag off, its default, and a `.pkg`/archive ROM is refused outright rather
than re-extracted and thrown away on every launch, so only natively bootable
formats work. While extracting, `GET /api/session/status` reports
`extraction_phase` as `"extracting_archive"` or `"extracting_pkg"`.
</Callout>
