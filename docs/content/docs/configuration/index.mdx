...
| `SAVE_FILE_MAX_BYTES` | `268435456` | Ceiling on a save or memory-card archive, over the import, exit-dump and memory-card routes (256 MiB). Unlike the other broker settings this one has no `BROKER_` prefix. |
| `BROKER_SAVE_FILE_MAX_ENTRIES` | `10000` | Ceiling on the number of members a save or memory-card archive may contain. Independent of `SAVE_FILE_MAX_BYTES`: a byte-size cap alone doesn't stop an archive of huge numbers of near-zero-byte, deeply nested entries from exhausting inodes or hanging the restore walk. |
| `BROKER_GAMEPAD_SLOTS` | `4` | Number of virtual gamepad slots the room can assign. |
| `BROKER_MAX_ROOM_VIEWERS` | `32` | Ceiling on concurrent viewer seats in the room. A disconnected anonymous seat is reclaimed for a new arrival once the ceiling is hit; a named user's seat is never reclaimed. |
