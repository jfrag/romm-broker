...
| `SHADPS4_CACHE_ENABLED` | `false` | Keep a `.pkg` extraction in `SHADPS4_CACHE_DIR` and reuse it on the next launch instead of deleting it once the game stops. |
...
| `SHADPS4_CACHE_MAX_GB` | `30` | Ceiling on the cache's total size; the oldest extraction is evicted first once a new one would exceed it. |
...
| `RPCS3_CACHE_ENABLED` | `false` | Keep a PKG/archive extraction in `RPCS3_CACHE_DIR` and reuse it on the next launch instead of deleting it once the game stops. |
| `RPCS3_CACHE_DIR` | `/config/.config/rpcs3/extracted` | Where a kept extraction is cached, when `RPCS3_CACHE_ENABLED` is on. |
| `RPCS3_CACHE_MAX_GB` | `30` | Ceiling on the cache's total size; the oldest extraction is evicted first once a new one would exceed it. |
