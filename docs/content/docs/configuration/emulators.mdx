...
| `SCUMMVM_FULLSCREEN` | `false` | Let ScummVM fill the stream. A window is the game's own resolution, 640x480 for most, which is a postage stamp on a 1080p stream — but fullscreen makes SDL grab and confine the pointer against a stack that feeds it absolutely, so the cursor stops tracking while clicks still land. Off by default because these games are nothing but mouse; turn it on for an image known to survive it. |
