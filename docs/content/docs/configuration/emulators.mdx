...
| `SCUMMVM_FULLSCREEN` | `true` | Let ScummVM fill the stream. A window is the game's own resolution, 640x480 for most, which is a postage stamp on a 1080p stream. Set it false for a compositor that confines the pointer to a fullscreen window, since these games are nothing but mouse. |
